...
From this comparison, we can easily observe that DEASFN is better than the camera-based method in dark environment. DEASFN can correctly distinguish between the front of person and the back. However, the camera-based method is possible to get mixed up under poor illumination. Furthermore, Openpose is even likely to loss some keypoint, such as wrist, ankle, and so forth.
|**DEASFN**|Openpose|                       
| :------:| :------: |
