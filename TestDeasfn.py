    ...
    if DatasetName == 'SPE':
                ...
                for r in TestRange:
                    for index in r:
                        ...
                        for j in range(TestArgs.NumFrames):
                            data = hdf5storage.loadmat(TestArgs.DataPath +'SPE/'+subject+'/'+action+'/train/' + str(index - (TestArgs.NumFrames+1+j) * TestArgs.DilatedRate) + '.mat', variable_names={'csi_serial'})
        ...
        for path in TestArgs.GpeInfo:
            ...
            for index in tqdm(path[1], desc=path[0]):
                ...
                csi_data = Variable(csi_data.cuda())
                pred_JH_tensor, pred_PAF_tensor = wisppn(csi_data)
